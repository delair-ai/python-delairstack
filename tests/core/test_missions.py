...
from delairstack.core.resources import Resource
...
class TestMissions(ResourcesTestBase):
    ...
    def test_create_mission(self):
        ...
        project = Project(id='project_id', desc={'_id': 'project_id'})
    ...
    @responses.activate
    def test_search_without_error_legacy(self):
        responses.add('POST', '/dxpm/missions/search',
                      body=self.__search_post_response(), status=200,
                      content_type='application/json')
        ...
        with self.assertWarns(DeprecationWarning):
            self.sdk.missions.search(name='My name')
            self.assertEqual(len(calls), 1)
            self.assertEqual(calls[0].request.url, '/dxpm/missions/search')
            self.assertEqual(
                calls[0].request.body, '{"search": "My name"}')

        with self.assertWarns(DeprecationWarning):
            self.sdk.missions.search(name='My name', project='project-id')
            self.assertEqual(len(calls), 2)
            self.assertEqual(calls[1].request.url, '/dxpm/missions/search')
            self.assertDictEqual(
                json.loads(calls[1].request.body),
                {"search": "My name", "project_id": "project-id"})
    ...
    def test_complete_survey_upload(self):
        ...
        flight = Resource(id=obj['_id'], desc=obj)
